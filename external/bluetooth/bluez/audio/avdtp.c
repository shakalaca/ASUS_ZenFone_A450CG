...
static void set_disconnect_timer(struct avdtp *session, int timeout)
{
	...
	session->dc_timer = g_timeout_add_seconds(timeout, disconnect_timeout,
						session);
}
...
static void connection_lost(struct avdtp *session, int err)
{
	...
	DBG("Disconnected from %s error %d", address,err);
...
void avdtp_unref(struct avdtp *session)
{
	...
	if (session->ref < 0) //something going wrong
		return;
	...
	if (session->ref == 1) {
		...
		if (session->io)
			set_disconnect_timer(session, DISCONNECT_IMMEDIATE);
				...
				gpointer data)
{
	...
	if (cond & (G_IO_HUP | G_IO_ERR)){		
		DBG("session reference count is %d",session->ref);
		session->ref = 1; // forcing cleanup
		goto failed;
		}
...
static void avdtp_connect_cb(GIOChannel *chan, GError *err, gpointer user_data)
{
	...
	/* The following sleep is a workaround for some headsets
	 * which try to connect at the same time as we do and
	 * report the media resources as being in use
	 */
	DBG("sleeping 500 ms");
	usleep(500 * 1000);
