...
#define BUF_NUM_PERIODS 4
/* factor for determining buffer size */
#define BUF_SIZE 8
/*
 * factor for evaluating the expected time in usecs for writing a byte from the
 * PCM buffer to A2DP, used to determine the sleeping time of the writing
 * thread by comparison with the elapsed time
 */
#define BUF_WRITE_EXP_TIME 1750
...
{
    const struct astream_out *out = (const struct astream_out *)stream;
    return out->buffer_size * BUF_NUM_PERIODS;
}
    ...
    while(!out->buf_thread_exit) {
	    ...
	    int skip_flow_control = 0;
            ...
            while (frames > 0 && !out->buf_thread_exit) {
                ...
                buffer_duration_us = ((ret * 1000) / out->sample_rate) *
                                     BUF_WRITE_EXP_TIME;
                skip_flow_control = get_flow_check();  
                if (skip_flow_control == 0 && elapsed_us < (buffer_duration_us / 4)) {
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        ...
        if (adev->output && adev->output->standby)
