...
int decrease_bitpool = 0;
int avdtp_good_counter = 0;
int skip_avdtp_flow_check = 0;
int get_flow_check(){
 return skip_avdtp_flow_check;
}
...
static int bluetooth_start(struct bluetooth_data *data)
{
	...
	err = getsockopt(data->stream.fd, SOL_L2CAP, L2CAP_CONNINFO,
				&info, &infoSize);
	if (err < 0)	{
		ERR("Can't get L2CAP options: failed %d", err);
		goto error;
	}
    ...
    if(data->bitpool){
		...
		def_bitpool = (data->isEdrCapable) ?
				...
				medium_quality_default_bitpool(cap->frequency,
								cap->channel_mode);
		data->bitpool=def_bitpool;
	}
               
		min_bitpool = MAX(MIN_BITPOOL, cap->min_bitpool);
		max_bitpool = MIN(def_bitpool, cap->max_bitpool);
        DBG("max_bitpool bitpool %d isEdrCapable %d",def_bitpool,data->isEdrCapable); 
...
int a2dp_write(a2dpData d, const void* buffer, int count)
{
	...
	while (frames_left >= codesize) {
		...
		VDBG("current bitpool %d ",data->sbc.bitpool);
