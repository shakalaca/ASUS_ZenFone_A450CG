					...
					void *user_data)
{
	...
	struct a2dp_data *a2dp = &client->d.a2dp;
	uint16_t version;
	char src_addr[18], dst_addr[18],name[249]="";
	...
	rsp->isEdrCapable = a2dp_read_edrcapability(&client->dev->src,
							&client->dev->dst);
	ba2str(&client->dev->src, src_addr);
	ba2str(&client->dev->dst, dst_addr);
	read_device_name(src_addr, dst_addr, name);

	if(strncmp(name, "HBH-DS970", 9)/* not equal */){
	rsp->bitpool=0;
	}else{
	rsp->bitpool=32;
	}	
	
	DBG("%s EdrCapable, %d. bitpool, %d", name, rsp->isEdrCapable, rsp->bitpool);
