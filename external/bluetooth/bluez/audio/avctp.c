...
{
	struct audio_device *dev;
	char address[18], name[248 + 1], uinput_dev_name[10]="AVRCP";
...
static void avctp_confirm_cb(GIOChannel *chan, gpointer data)
{
	...
	if (session->state == AVCTP_STATE_CONNECTING || session->state == AVCTP_STATE_CONNECTED){
		DBG("AVCTP already connect to a device.");
		return;
	}
	if (!session)
		goto drop;
