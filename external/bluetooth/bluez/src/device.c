...
static uint16_t uuid_list_msi_fs300[] = {
					...
					DBusMessage *msg, void *user_data)
{
	...
	if (strlen(pattern) == 0) {
		err = device_browse_sdp(device, conn, msg, NULL, TRUE);
		if (err < 0)
			goto fail;
	} else {
		uuid_t uuid;
...
static void browse_cb(sdp_list_t *recs, int err, gpointer user_data)
{
	...
	bdaddr_t src;
	uuid_t uuid;
	int search_uuid_size=2;

	/* workaround for MSI FS300 headset */
	DBG("BT device name: %s", device->name);
	if(strncmp(device->name, "MSI FS300", 9)/* not equal */){
		search_uuid_size=2;
	}else{
		DBG("workaround for MSI FS300 headset: search_uuid_size=4");
		search_uuid_size=4;
	}

	/* If we have a valid response and req->search_uuid == 2, then L2CAP
	 * UUID & PNP searching was successful -- we are done */
	if (err < 0 || (req->search_uuid == search_uuid_size && req->records)) {
	...
	if(strncmp(device->name, "MSI FS300", 9)/* not equal */){
		...
		DBG("workaround for MSI FS300 headset: use uuid_list_msi_fs300");
		if (uuid_list_msi_fs300[req->search_uuid]) {
			sdp_uuid16_create(&uuid, uuid_list_msi_fs300[req->search_uuid++]);
			bt_search_service(&src, &device->bdaddr, &uuid,
							browse_cb, user_data, NULL);
			return;
		}
