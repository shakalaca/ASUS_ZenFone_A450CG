...
int ctp_soc_jack_gpio_detect(void)
{
	...
	if (!enable) {
		...
		pr_info("h2w : headset/headphone connected\n");
		schedule_delayed_work(&ctx->jack_work_insert,
					HPDETECT_POLL_INTERVAL);
	} else {
		pr_info("h2w : headset/headphone disconnected\n");
...
static int snd_ctp_mc_probe(struct platform_device *pdev)
{
	...
	struct ctp_mc_private *ctx;
        struct snd_soc_jack_gpio *gpio = &hs_gpio[CTP_HSDET_GPIO];
        struct snd_soc_jack_gpio *gpio2 = &hs_gpio[CTP_BTN_GPIO];
	...
	ret_val = snd_ctp_register_jack_data(pdev, ctx);
        pr_info("h2w %s() : hsdet gpio1_number:%d\n", __func__, gpio->gpio);
        pr_info("h2w %s() : hook gpio1_number:%d\n", __func__, gpio2->gpio);
	if (ret_val)
		goto free_jack;
