...
/* #define USE_INT_CLK */
/* #define JD1_FUNC */ // joe_cheng : remove JD function
/* #define ALC_DRC_FUNC */
#define USE_ASRC
#define SET_PATH
...
static int Hal_user_current_device;

static struct rt5647_init_reg init_list[] = {
        { RT5647_DAC2_DIG_VOL   , 0xaeae },
        { RT5647_ADJ_HPF2       , 0x0606 },
        { RT5647_SPO_CLSD_RATIO , 0x0005 },
	...
	// joe_cheng : disable lineout path
	{ RT5647_OUTMIXL_CTRL3	, 0x01ff },/* DACL1 -> OUTMIXL */
	{ RT5647_OUTMIXR_CTRL3	, 0x01ff },/* DACR1 -> OUTMIXR */
...
int rt5647_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	if (jack_insert) {
		...
		snd_soc_write(codec, RT5647_GEN_CTRL2, 0x0000);
		...
		default:
			...
			snd_soc_update_bits(codec, RT5647_CJ_CTRL2, 0x80, 0x0);
...
int rt5647_button_detect(struct snd_soc_codec *codec)
{
	...
	/*
	// joe_cheng : remove for button press/release correct
	...
	*/
	...
	btn_type = val & 0xff80;
	pr_debug("%s : MX-DB 0x%x btn_type=0x%x\n", __func__, val, btn_type);
	...
	*/
	...
	pr_debug("%s : return btn_type 0x%x\n", __func__, btn_type);
	if (btn_type == 0x400)
		return 1;
	else if (btn_type == 0x0)
		return 2;
	else
		return 0;
}
...
static const char *rt5647_scenario_mode[] = {"Normal", "Playback", "Record", "Voip", "RingTone", "VR", "Voice"};
static const char *rt5647_scenario_device[] = {"none", "speaker", "receiver", "headphone", "dmic", "headsetmic", "sco_headset"};

static const SOC_ENUM_SINGLE_DECL(rt5647_user_scenario_enum, 0, 0, rt5647_scenario_mode);
static const SOC_ENUM_SINGLE_DECL(rt5647_user_device_enum, 0, 0, rt5647_scenario_device);

static int rt5647_scenario_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("rt5647_scenario_get: value %d\n",Hal_user_scenario_mode);

	return 0;
}

static int rt5647_scenario_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5647_priv *rt5647 = snd_soc_codec_get_drvdata(codec);

	pr_debug("rt5647_scenario_put: value %d\n",ucontrol->value.integer.value[0]);
...
static int rt5647_device_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5647_priv *rt5647 = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.integer.value[0] = Hal_user_current_device;
	pr_debug("%s: value %d\n", __func__, Hal_user_current_device);

	return 0;
}

static int rt5647_device_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5647_priv *rt5647 = snd_soc_codec_get_drvdata(codec);
	...
	if (Hal_user_current_device == ucontrol->value.integer.value[0])
		return 0;

	Hal_user_current_device = ucontrol->value.integer.value[0];
...
static const struct snd_kcontrol_new rt5647_snd_controls[] = {
	...
	SOC_DOUBLE("DMIC2 Latch Switch", RT5647_DMIC_CTRL2, 3, 2, 1, 0),
	...
	SOC_SINGLE("I2S2 Func Switch", RT5647_GPIO_CTRL1,
        ...
        SOC_ENUM_EXT("Scenario", rt5647_user_scenario_enum, 
			rt5647_scenario_get, rt5647_scenario_put),
        SOC_ENUM_EXT("Scenario device", rt5647_user_device_enum, 
			rt5647_device_get, rt5647_device_put),
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_debug("%s : event %d, mode %d device %d\n", __func__, event, Hal_user_scenario_mode, Hal_user_current_device);
...
static void rt5647_pmu_depop(struct snd_soc_codec *codec)
{
	...
	// joe_cheng : remove this workqueue since the pop sound is fixed by 
	// another Bard's patch. As a result, the first short tone in headset would not be muted.
//	schedule_delayed_work(&hp_work,
//					msecs_to_jiffies(300));
	snd_soc_update_bits(codec, RT5647_HP_VOL, RT5647_L_MUTE | RT5647_R_MUTE, 0);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_update_bits(codec, RT5647_PWR_DIG1,
			...
			RT5647_PWR_CLS_D | RT5647_PWR_CLS_D_R | RT5647_PWR_CLS_D_L);
		// joe_cheng : enable mclk detection to protect speaker
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		if ((Hal_user_scenario_mode == PLAYBACK || Hal_user_scenario_mode == RINGTONE) && Hal_user_current_device == SPEAKER) {
			rt5647_update_eqmode(codec, EQ_CH_DACL, PLAYBACK);
		} 
		
		if (Hal_user_scenario_mode == RINGTONE && Hal_user_current_device == SPEAKER) {
			pr_debug("%s : rt5647_update_drc_mode 1\n", __func__);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		if ((Hal_user_scenario_mode == PLAYBACK || Hal_user_scenario_mode == RINGTONE) && Hal_user_current_device == SPEAKER) {
			rt5647_update_eqmode(codec, EQ_CH_DACR, PLAYBACK);
		}
		if (Hal_user_scenario_mode == RINGTONE && Hal_user_current_device == SPEAKER) {
			pr_debug("%s : rt5647_update_drc_mode 1\n", __func__);
			rt5647_update_drc_mode(codec, 1);
...
static const struct snd_soc_dapm_route rt5647_dapm_routes[] = {
	...
	{"INL VOL", NULL, "dac stereo1 filter"},
	{"INR VOL", NULL, "dac stereo1 filter"},
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	printk("%s : hw_params:DAI ID is %d\n", __func__, dai->id);
...
static DEVICE_ATTR(index_reg, 0644, rt5647_index_show, rt5647_index_store);
...
static DEVICE_ATTR(codec_reg, 0644, rt5647_codec_show, rt5647_codec_store);
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_OFF:
		...
		snd_soc_write(codec, RT5647_DIG_MISC, 0x2060);
