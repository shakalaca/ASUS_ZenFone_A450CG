...
static int kxtj9_into_drdy_function(struct kxtj9_data *tj9)
{
	...
	if(KXTJ9_DEBUG_MESSAGE) printk("alp : kxtj9_into_drdy_function (%d)\n",tj9->ctrl_reg1);
...
static int kxtj9_into_motion_detect_function(struct kxtj9_data *tj9)
{
	...
	if(KXTJ9_DEBUG_MESSAGE) printk("alp : kxtj9_into_motion_detect_function (%d)\n",tj9->ctrl_reg1);
	result = i2c_smbus_write_byte_data(tj9->client, CTRL_REG1, tj9->ctrl_reg1); // enable motion detect
	if (result < 0){
...
static ssize_t enableirq_kxtj9(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kxtj9_data *tj9 = i2c_get_clientdata(client);	
	enable_irq(tj9->irq);
...
static DEVICE_ATTR(enableirq, 0660, enableirq_kxtj9, NULL);
