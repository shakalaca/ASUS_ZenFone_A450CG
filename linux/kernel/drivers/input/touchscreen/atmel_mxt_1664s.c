...
#define NEWEST_FW_VERSION_1664T 0x12
#define NEWEST_FW_BUILD_1664T 0xaa
	...
	if(chip_in_bootloader==1 && build_version!=1) {
		...
		printk("[Ateml] mxt1664 probe mxt_initialize END\n");
	        error = request_threaded_irq(touch_chip->irq, NULL, mxt_interrupt,
                	        IRQF_TRIGGER_FALLING | IRQF_ONESHOT, touch_chip->client->dev.driver->name, touch_chip); // Modified
...
static ssize_t mxt_enable_touch(struct device *dev,struct device_attribute *attr, char *buf){
        struct mxt_data *data = touch_chip;
        int error = 0;
	...
	error = enable_touch();
        ...
        int count = 0;
        count += snprintf(buf + count, PAGE_SIZE - count, "%s\n", error ? "Enable touch fail" : "Enable touch success");
        if (count >= PAGE_SIZE)
                        return PAGE_SIZE - 1;
...
static ssize_t mxt_disable_touch(struct device *dev,struct device_attribute *attr, char *buf){
        struct mxt_data *data = touch_chip;
        int error = 0;
        ...
        int count = 0;
        count += snprintf(buf + count, PAGE_SIZE - count, "%s\n", error ? "Disable touch fail" : "Disable touch success");
        if (count >= PAGE_SIZE)
                        return PAGE_SIZE - 1;
...
static DEVICE_ATTR(disable_touch, 0444, mxt_disable_touch, NULL);
static DEVICE_ATTR(enable_touch, 0444, mxt_enable_touch, NULL);
...
//	irq_is_enable = 1;
	...
	error = request_threaded_irq(data->irq, NULL, mxt_interrupt,
			IRQF_TRIGGER_FALLING | IRQF_ONESHOT, client->dev.driver->name, data); // Modified
	printk("[Ateml] mxt1664 probe request_threaded_irq\n");
