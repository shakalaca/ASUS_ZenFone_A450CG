...
static int himax_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if(!himax_data->tp_firmware_upgrade_proceed)
	{
		...
		if (ret)
		{
			printk(KERN_INFO "[Himax] %s: cancel_work_sync = %d\n",__func__,ret);
...
static ssize_t himax_get_fw_version(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk(KERN_INFO "[Himax]: D%s.%s.%s-%s\n",himax_data->himax_firmware_version, himax_data->tp_lens_version, himax_data->config_firmware_version,DRIVER_VERSION);
...
static ssize_t himax_me372cl_chip_proc_diag_write(struct file *filp, const char __user *buf, unsigned long len, void *data)
{
	...
	{       
		...
		{
			printk(KERN_INFO "[Himax_ME372CL] %s: coordinate_dump_file_create error\n", __func__);
...
static ssize_t himax_me372cl_chip_proc_register_read(struct seq_file *buf, void *v)
{
	...
	if(config_bank_reg)
	{
		printk(KERN_INFO "[Himax_ME372CL] %s: register_command = FE(%x)\n", __func__, register_command);
...
static int himax_touch_sysfs_init(void)
{
	...
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_himax_check_sum.attr);
	if (ret)
	{
...
//----[HX_RST_PIN_FUNC]---------------------------------------------------------------------------------end
	...
	himax_me372cl_cable_status(cable_status);
	// add by josh for check USB status ---
...
EXPORT_SYMBOL(himax_me372cl_cable_status);
