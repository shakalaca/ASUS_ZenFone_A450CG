...
#define DRIVER_VERSION	"1.0.5"		//Joe modify 
...
#define ASUSEC_VERIFY_PATH      "/data/logtool/EC_FW_Verify.bin" // add by tom for debug 
...
static int output_EC_FW()
{
	...
	if(!IS_ERR_OR_NULL(filePtr))
	{
		oldfs = get_fs();
		set_fs(get_ds());
		ret = filePtr->f_op->write(filePtr, &upgrade_fw, 1024*64, &pos);
		set_fs(oldfs);
		filp_close(filePtr, NULL);
		printk("[ITE] %s: write %s done. ret = %d \n", __func__, ASUSEC_VERIFY_PATH,ret);
	}
	return 0;
}
...
int ite_firmware_upgrade(int path)
{
	...
	u8 als_cali_value_ACD[2] = {0}; // add by tom for get ALS calibration value from ALS ACD
...
//	printk("[ITE_update]: ite_chip_data->version_checksum = %d \n",ite_chip_data->version_checksum);
	...
	{	
		...
		else if(upgrade_status == 1 && als_cali_value_EC[2] != 0xFF && als_cali_value_EC[3] != 0xFF)
		{
