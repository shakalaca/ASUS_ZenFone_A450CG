...
#endif
#if CONFIG_TOUCHSCREEN_HIMAX_HX8528_A12
extern int himax_a12_cable_status(int status);
#endif
#if CONFIG_TOUCHSCREEN_HIMAX_HX8528_P72G
extern int himax_cable_status_p72g(int status);
#endif
#if CONFIG_TOUCHSCREEN_HIMAX_HX8528_A400
extern int himax_a400cg_cable_status(int status);
#endif
...
inline void notify_cable_status(int type)
{
	...
	printk("USB : notify cable status = %d \n",type);
	...
	if (entry_mode ==1){
		...
		#endif
		#if CONFIG_TOUCHSCREEN_HIMAX_HX8528_A12
		himax_a12_cable_status(type);
		#endif
		#if CONFIG_TOUCHSCREEN_HIMAX_HX8528_P72G
		himax_cable_status_p72g(type);
		#endif
		#if CONFIG_TOUCHSCREEN_HIMAX_HX8528_A400
		himax_a400cg_cable_status(type);
		#endif
		#if CONFIG_TOUCHSCREEN_HIMAX_HX8528_A450CG
...
/* This function transfer USB charging type and current limited
 * to asus specific charing type that acceptable by charger driver.
 *
 * In current implement, charger accepts USB_IN, AC_IN and CABLE_OUT.
 * current limit is handled by itself. */
static void asus_update_chrg_cap(struct penwell_otg *pnw,
                                 enum usb_charger_type charger,
                                 unsigned mA)
{

    switch (charger) {
        case CHRG_SDP_INVAL:	/* Invaild Standard Downstream Port */
            dev_warn(pnw->dev, "abnormal charger type: %s\n", charger_string(charger));
        case CHRG_SDP:          /* Standard Downstream Port */
            if (mA < CHRG_CURR_SDP_HIGH)
                dev_warn(pnw->dev, "request: %u mA, but treat as USB_IN/SDP\n", mA);
            notify_cable_status(USB_IN);
            break;
        case CHRG_CDP:          /* Charging Downstream Port */
        case CHRG_DCP:          /* Dedicated Charging Port */
        case CHRG_ACA:          /* Accessory Charger Adapter */
        case CHRG_ACA_DOCK:     /* Accessory Charger Adapter - Dock */
        case CHRG_ACA_A:        /* Accessory Charger Adapter - RID_A */
        case CHRG_ACA_B:        /* Accessory Charger Adapter - RID_B */
        case CHRG_ACA_C:        /* Accessory Charger Adapter - RID_C */
        case CHRG_SE1:          /* SE1 (Apple)*/
        case CHRG_MHL:          /* Moblie High-Definition Link */
            if (mA < CHRG_CURR_CDP)
                dev_warn(pnw->dev, "request: %u mA, but treat as AC_IN/DCP\n", mA);
				...
				unsigned ma)
{
	...
	asus_update_chrg_cap( pnw, charger, ma);
		...
		const struct pci_device_id *id)
{
	...
	if (Read_PROJ_ID()==PROJ_ID_A450CG)
