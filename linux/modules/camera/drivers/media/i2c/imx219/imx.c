...
int ATD_imx219_status = 0;
static char camera_module_otp[60];

static ssize_t imx219_show_status(struct device *dev,struct device_attribute *attr,char *buf)
{
	printk("%s: get imx219 status (%d) !!\n", __func__, ATD_imx219_status);

	return sprintf(buf,"%d\n", ATD_imx219_status);
}

static ssize_t imx219_read_otp(struct device *dev,struct device_attribute *attr,char *buf)
{
	printk("%s: get imx219 module OTP %s !!\n", __func__, camera_module_otp);

	return sprintf(buf,"%s", camera_module_otp);
}

static DEVICE_ATTR(imx219_status, S_IRUGO,imx219_show_status,NULL);
static DEVICE_ATTR(imx219_read_otp, S_IRUGO,imx219_read_otp,NULL);

static struct attribute *imx219_attributes[] = {
	&dev_attr_imx219_status.attr,
	&dev_attr_imx219_read_otp.attr,
	NULL
};
//Add for ATD command---
...
static int __imx_otp_read(struct v4l2_subdev *sd, struct imx_af_data *buf)
{
	...
	//Check lite-on 3rd Data Package: Page2/Row0/0x3204 to Page2/Row1/0x321b
	/*set page NO.*/
	ret = imx_write_reg(client, IMX_8BIT, IMX_OTP_PAGE_REG, 2);
	if (ret) {
	...
	printk("%s Check lite-on 3rd Data Package 0x%X 0x%X\n", __func__, read_value[0], read_value[1]);
	if((read_value[0]!=0 || read_value[1]!=0) && (read_value[0]!=0xff || read_value[1]!=0xff))
		goto out;


	//Check chicony 3rd Data Package: Page0/Row3/0x3234 to Page1/ Row0/0x320b
	/*set page NO.*/
	...
	/* Reading the OTP data array */
	ret = imx_read_otp_reg_array(client, 16, 0x3234, read_value);
	if (ret) {
		dev_err(&client->dev, "failed to read OTP data\n");
		return ret;
	}

	printk("%s Check chicony 3rd Data Package 0x%X 0x%X\n", __func__, read_value[0], read_value[1]);
	if((read_value[0]!=0 || read_value[1]!=0) && (read_value[0]!=0xff || read_value[1]!=0xff)) {
		...
		/* Reading the OTP data array */
		ret = imx_read_otp_reg_array(client, 8, IMX_OTP_START_ADDR, read_value+16);
		if (ret) {
			dev_err(&client->dev, "failed to read OTP data\n");
			return ret;
		}
	...
	//Check lite-on 2rd Data Package: Page1/Row0/0x3204 to Page1/Row1/0x321b
	/*set page NO.*/
	ret = imx_write_reg(client, IMX_8BIT, IMX_OTP_PAGE_REG, 1);
	if (ret) {
		dev_err(&client->dev, "failed to prepare OTP page\n");
	...
	printk("%s Check lite-on 2rd Data Package 0x%X 0x%X\n", __func__, read_value[0], read_value[1]);
	if((read_value[0]!=0 || read_value[1]!=0) && (read_value[0]!=0xff || read_value[1]!=0xff))
		goto out;

	//Check chicony 2rd Data Package: Page0/Row1/0x321c to Page0/Row2/0x3233
	/*set page NO.*/
	ret = imx_write_reg(client, IMX_8BIT, IMX_OTP_PAGE_REG, 0);
	if (ret) {
		dev_err(&client->dev, "failed to prepare OTP page\n");
	...
	printk("%s Check chicony 2rd Data Package 0x%X 0x%X\n", __func__, read_value[0], read_value[1]);
	if((read_value[0]!=0 || read_value[1]!=0) && (read_value[0]!=0xff || read_value[1]!=0xff))
		goto out;

	//Check lite-on / chicony 1rd Data Package: Page0/Row0/0x3204 to Page0/Row1/0x321b
	/*set page NO.*/
	ret = imx_write_reg(client, IMX_8BIT, IMX_OTP_PAGE_REG, 0);
	if (ret) {
		dev_err(&client->dev, "failed to prepare OTP page\n");
	...
	printk("%s Check lite-on / chicony 1rd Data Package 0x%X 0x%X\n", __func__, read_value[0], read_value[1]);
	if((read_value[0]!=0 || read_value[1]!=0) && (read_value[0]!=0xff || read_value[1]!=0xff))
		goto out;
	...
	u16 gain, u16 digitgain)
{
	...
	if(dev->run_mode != CI_MODE_VIDEO) {
                if (dev->curr_res_table[dev->fmt_idx].bin_factor_x <= 1 &&
...
int imx_s_hilight(struct v4l2_subdev *sd, s32 value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
{
    s32 vcm_steps;
    imx_q_focus_abs(main_sd, &vcm_steps);
...
static ssize_t camera_proc_write(struct file *filp, const char __user *buff,
            size_t len, loff_t *data)
{
    ...
    if (!strncmp("setabs",buffer,6)) {
        ...
        imx_t_focus_abs(main_sd, value);
    } else if(!strncmp("setrel",buffer,6)){
        sscanf(&buffer[7],"%d",&value);
        printk("add position %d\n", value);
        imx_t_focus_rel(main_sd, value);
    } else if (!strncmp("set_torch_on",buffer,12)) {
        printk("set torch on\n");
        gpio_set_value(flash_ent, 0);
        gpio_set_value(flash_enf, 0);
        msleep(10);
...
//For LED FLASH Control+++
static int imx219_s_flash_mode(struct v4l2_subdev *sd, s32 value)
{
	printk("imx219_s_flash_mode\n");
...
static int imx219_s_flash_timeout(struct v4l2_subdev *sd, s32 value)
{
	//Do nothing now
	return 0;
}

static int imx219_s_flash_request(struct v4l2_subdev *sd, s32 value)
{
	printk("imx219_s_flash_request\n");
...
static int imx219_s_flash_intensity(struct v4l2_subdev *sd, s32 value)
{
	//Do nothing now
	return 0;
}

static int imx219_s_torch_intensity(struct v4l2_subdev *sd, s32 value)
{
	//Do nothing now
	return 0;
}

static int imx219_s_indicator_intensity(struct v4l2_subdev *sd, s32 value)
{
	//Do nothing now
	return 0;
}
...
//For LED FLASH Control+++
	...
	},
	{
		...
		.tweak = imx219_s_flash_intensity,
	},
	{
		.qc = {
		...
		.tweak = imx219_s_torch_intensity,
	},
	{
		.qc = {
		...
		.tweak = imx219_s_indicator_intensity,
	},
//For LED FLASH Control---
};
...
//	u16 id;
	...
	v4l2_info(client, "imx_s_stream enable %d\n", enable);

	mutex_lock(&dev->input_lock);
	if (enable) {
//imx_read_reg(client, IMX_16BIT, IMX219_CHIP_ID, &id);
//v4l2_info(client, "sensor id = 0x%x\n", id);
			 ...
			 const struct i2c_device_id *id)
{
	...
	CAM_ID = gpio_get_value(114);
	printk("%s: CAM_ID %d\n", __func__, CAM_ID);
	gpio_free(114);

	//SUB_CAM_ID => 1 -> 8M : 0 ->5M
	if(CAM_ID==0) {
	...
	create_camera_proc_file(); //For DIT VCM Debug and ATD flash Interface+++

	//Add for ATD command+++
	dev->sensor_i2c_attribute.attrs = imx219_attributes;
