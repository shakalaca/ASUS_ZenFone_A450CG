...
static const struct file_operations host_wake_fops = {
        .owner = THIS_MODULE,
...
static int bcm_bt_lpm_init(struct platform_device *pdev)
{
	...
	bcm_bt_lpm_wake_peer(tty_dev);
        /* Create Bt_Wake_File */
	Bt_Wake_File = proc_create(PROCFS_NAME, S_IRUGO | S_IFREG, NULL, &host_wake_fops);
	
	if (Bt_Wake_File == NULL) {
		pr_err("%s: create proc node %s fail \n", __func__, PROCFS_NAME);
		return -ENOMEM;
	}
