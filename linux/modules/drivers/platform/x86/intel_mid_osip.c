	...
	if (data && 0 == strncmp(cmd, "recovery", 9)) {
		...
		if (ret_ipc < 0)
			pr_err("%s cannot write reboot reason in OSNIB\n",
				__func__);
		ret = NOTIFY_OK;
    } else if (data && 0 == strncmp(cmd, "factory", strlen("factory")) && build_version == 1) {
        //Will add for ADB reboot, setting reboot reason is RECOVERY  (only support eng)
        pr_warn("[SHTDWN] %s, rebooting into factory(miniOS)\n", __func__);
